...
void AgentModel::initBody()
{
  ...
  //Get the d_playerClass for this unum
  ostringstream xpath;
  ...
  d_playerClass = conf.selectXPath(xpath.str()).front().getProp("class");
  
  string path = "/conf/player-class[@id='" + d_playerClass + "']/agentmodel";
  ...
  path = "/conf/player-class[@id='" + d_playerClass + "']/agentmodel/bodypart[@name='torso']";
  set = conf.selectXPath(path);
  if (set.empty())
    throw runtime_error(string() + "AgentModel::initBody: '" + path + "' not found.");
  
  XMLNode torso = set.front();
  d_weight = initBody(torso, d_playerClass, 0);
