...
void SimpleLocalizer::update()
{
  ...
  d_locationsRaw[Types::BALL]->init(ballPos, zeroMat);

  d_locationsRaw[Types::GOAL1L]->init(goal1LPos, zeroMat);
  d_locationsRaw[team ==Types::LEFT ? Types::GOAL1US : Types::GOAL2THEM]->init(goal1LPos, zeroMat);
  
  d_locationsRaw[Types::GOAL2L]->init(goal2LPos, zeroMat);
  d_locationsRaw[team ==Types::LEFT ? Types::GOAL2US : Types::GOAL1THEM]->init(goal2LPos, zeroMat);
   
  d_locationsRaw[Types::GOAL1R]->init(goal1RPos, zeroMat);
  d_locationsRaw[team ==Types::LEFT ? Types::GOAL1THEM : Types::GOAL2US]->init(goal1RPos, zeroMat);
   
  d_locationsRaw[Types::GOAL2R]->init(goal2RPos, zeroMat);
  d_locationsRaw[team ==Types::LEFT ? Types::GOAL2THEM : Types::GOAL1US]->init(goal2RPos, zeroMat);

  d_locationsRaw[Types::FLAG1L]->init(flag1LPos, zeroMat);
  d_locationsRaw[team == Types::LEFT ? Types::FLAG1US : Types::FLAG2THEM]->init(flag1LPos, zeroMat);
  
  d_locationsRaw[Types::FLAG2L]->init(flag2LPos, zeroMat);
  d_locationsRaw[team ==Types::LEFT ? Types::FLAG2US : Types::FLAG1THEM]->init(flag2LPos, zeroMat);
   
  d_locationsRaw[Types::FLAG1R]->init(flag1RPos, zeroMat);
  d_locationsRaw[team ==Types::LEFT ? Types::FLAG1THEM : Types::FLAG2US]->init(flag1RPos, zeroMat);
   
  d_locationsRaw[Types::FLAG2R]->init(flag2RPos, zeroMat);
  d_locationsRaw[team ==Types::LEFT ? Types::FLAG2THEM : Types::FLAG1US]->init(flag2RPos, zeroMat);

  //5 players
  for (unsigned i = 0; i < 5; ++i) {
    Vector3d playerPosPolar = cochlea.getInfo((Cochlea::InfoID)(Cochlea::iVisionPlayer1 + i)).start<3>();
    if (playerPosPolar[0] != 0)
    {
      Vector3d playerPosCart = Math::polarToCartesian(playerPosPolar);
      d_locationsRaw[Types::PLAYER1 + i]->init(playerPosCart, zeroMat);
    }
    else
      d_locationsRaw[Types::PLAYER1 + i]->init(Vector3d(0,0,0), zeroMat);
  }
  //5 players
  for (unsigned i = 0; i < 5; ++i) {
    ...
    if (opponentPosPolar[0] != 0)
    {
      ...
      d_locationsRaw[Types::OPPONENT1 + i]->init(opponentPosCart, zeroMat);
    }
    else
      d_locationsRaw[Types::OPPONENT1 + i]->init(Vector3d(0,0,0), zeroMat);
  }
  ...
  for (unsigned i = 0; i < Types::NOBJECTS; ++i)
  {
    ...
    d_velocitiesLocal[i]->init((local - d_locationsLocal[i]->getMu()) / 0.02, zeroMat);
    d_locationsLocal[i]->init(local, zeroMat);
    
    Vector3d global = d_globalTransform * (Vector3d)d_locationsRaw[i]->getMu();
    d_locationsGlobal[i]->init(global, zeroMat);
  }
  ...
  for (unsigned i = 0; i < Types::NOBJECTS; ++i)
  {
    ...
    d_velocitiesGlobal[i]->init((global - d_locationsGlobal[i]->getMu()) / 0.02, zeroMat);
    d_locationsGlobal[i]->init(global, zeroMat);
  }
}
